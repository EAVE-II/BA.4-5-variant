...
output_list$a_end_hosp_expected <- as.Date("2022-06-19")
...
z_df <- df_pos %>% filter(date_ecoss_specimen >=  output_list$a_begin_analysis ) %>% 
  ...
  filter(in_eave==1) %>% 
  filter(lab != "lft") %>% 
  mutate(days_gp = fct_recode(days_gp, week_2="week_1", week_8="week_9"))
...
fmla.coxph <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~  days_gp +  age_gp + sex +
                               simd2020_sc_quintile + n_risk_gps + vs + prev_pos_gp"))
...
z.tab %>%  
  ...
  labs(x="Age Group", y="Rate Per 100 person years", colour="variant") + ylim(0,200)
...
fmla.coxph <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   days_gp + age_gp + sex +
                               simd2020_sc_quintile + n_risk_gps +  prev_pos_gp + variant + vs"))
z1 <- coxph(fmla.coxph, data=z_df)
summary(z1)
saveRDS(z1,"./output/coxph_model_ba2_ba5.rds")
fmla.coxph <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   days_gp + age_gp + sex +
                               simd2020_sc_quintile + n_risk_gps +  prev_pos_gp + variant + variant:vs"))
z1 <- coxph(fmla.coxph, data=z_df)
summary(z1)
...
z_df <- df_pos %>% filter(days_gp %in% c("week_6","week_7") ) %>% 
  filter(variant %in% c("omicron_ba2","omicron_ba5") ) %>% 
  filter(in_eave==1) %>% 
  filter(lab != "lft") %>% 
  filter(covid_admission_status != "covid_admission_admitted_before_test") 
...
z.tab %>%  
  ...
  geom_errorbar(aes(ymin=lower, ymax=upper, width=0.2), position = z_position) +
  labs(x="Age Group", y="Rate Per 1 person years", colour="variant") + ylim(0,16)
ggsave(paste0("./output/rate_variant_age_14days.png"), width=14, height=10, unit="cm")

# comparing ba5/ba2
#  Changes z_df

z_df <- df_pos %>% filter(date_ecoss_specimen >=  as.Date("2022-04-14") ) %>% filter(days_gp != "week_9") %>% 
  filter(variant %in% c("omicron_ba2","omicron_ba5") ) %>% 
  filter(in_eave==1) %>% 
  filter(In_Hosp_At_Test == "no") %>% 
  filter(date_ecoss_specimen <= output_list$a_end_hosp_14days_expected) 
...
z_tab_pred <- bind_cols(z_tab_pred, z_ci[,c("rate","lower","upper")])
write.csv(z_tab_pred, paste0("./output/pyears_by_variant_exp_hosp_eave_death.csv"), row.names = F)
...
z.tab %>%  
  ggplot(aes(x=age_gp, colour=variant)) + geom_point(aes(y=rate_100py), position=z_position) + 
  geom_errorbar(aes(ymin=lower, ymax=upper, width=0.2), position = z_position) +
  labs(x="Age Group", y="Rate Per 100 person years", colour="variant") + ylim(0,80)
